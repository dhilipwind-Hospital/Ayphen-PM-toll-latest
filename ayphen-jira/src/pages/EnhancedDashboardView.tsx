...
export const EnhancedDashboardView: React.FC = () => {
  ...
  const fetchDashboards = async () => {
    try {
      ...
      const response = await dashboardsV2Api.getAll(currentUser.id);
  ...
  const fetchIssues = async () => {
    try {
      ...
      const response = await issuesApi.getAll({ projectId: projectId || undefined, userId: userId || undefined });
  ...
  const createDefaultDashboard = async () => {
    try {
      ...
      const response = await dashboardsV2Api.create(dashboardData);
  ...
  const fetchGadgets = async (dashboardId: string) => {
    try {
      const response = await gadgetsApi.getByDashboard(dashboardId);
  ...
  const handleLayoutChange = async (layout: Layout[]) => {
    ...
    try {
      ...
      const updates = layout.map(l => {
        ...
        if (gadget) {
          return gadgetsApi.update(gadget.id, {
  ...
  const handleCreateGadget = async (values: any) => {
    try {
      ...
      const response = await gadgetsApi.create(newGadget);
  ...
  const handleRemoveGadget = async (gadgetId: string) => {
    try {
      await gadgetsApi.delete(gadgetId);
  ...
  const handleCloneDashboard = async () => {
    ...
    try {
      const currentUser = JSON.parse(localStorage.getItem('currentUser') || '{}');
      const response = await dashboardsV2Api.clone(currentDashboard.id, {
  ...
  const handleToggleStar = async () => {
    ...
    try {
      const response = await dashboardsV2Api.toggleStar(currentDashboard.id);
      setCurrentDashboard(response.data);
