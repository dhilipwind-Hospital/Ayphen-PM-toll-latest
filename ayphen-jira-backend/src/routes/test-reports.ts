...
router.get('/pass-rate', async (req, res) => {
  try {
    ...
    if (groupBy === 'suite') {
      ...
      const results = Object.values(bySuite).map((suite: any) => ({
        ...suite,
      ...
      const results = Object.values(byEnv).map((env: any) => ({
        ...env,
      ...
      // Group by browser
      const byBrowser: Record<string, any> = {};
      ...
      const results = Object.values(byBrowser).map((browser: any) => ({
        ...browser,
...
router.get('/flaky-tests', async (req, res) => {
  try {
    ...
    // Group by test case
    const byTestCase: Record<string, any> = {};
    ...
    results.forEach(result => {
      ...
      if (result.status === 'passed') byTestCase[testCaseId].passed++;
      if (result.status === 'failed') byTestCase[testCaseId].failed++;
    });

    // Identify flaky tests (tests with both passes and failures)
    const flakyTests = Object.values(byTestCase)
      .filter((tc: any) => tc.total >= Number(minRuns) && tc.passed > 0 && tc.failed > 0)
      .map((tc: any) => ({
...
router.get('/defects', async (req, res) => {
  try {
    ...
    const testRelatedBugs = bugs.filter(b => 
      b.labels && (b.labels.includes('test-failure') || b.labels.includes('auto-created'))
    );
