...
import { aiSmartPrioritizationService } from '../services/ai-smart-prioritization.service';
...
router.post('/analyze/:issueId', async (req, res) => {
  try {
    ...
    const result = await aiSmartPrioritizationService.analyzePriority(issueId);
...
router.post('/apply/:issueId', async (req, res) => {
  try {
    ...
    const analysis = await aiSmartPrioritizationService.analyzePriority(issueId);

    if (autoApply) {
      const updatedIssue = await aiSmartPrioritizationService.applyPriority(
        issueId,
...
router.post('/bulk-analyze', async (req, res) => {
  try {
    ...
    const results = await aiSmartPrioritizationService.bulkAnalyzePriority(issueIds);
...
router.post('/bulk-apply', async (req, res) => {
  try {
    ...
    const analyses = await aiSmartPrioritizationService.bulkAnalyzePriority(issueIds);
    ...
    for (const analysis of analyses) {
      if (analysis.shouldChange) {
        try {
          await aiSmartPrioritizationService.applyPriority(
            analysis.issueId,
            analysis.suggestedPriority
          );
          appliedCount++;
