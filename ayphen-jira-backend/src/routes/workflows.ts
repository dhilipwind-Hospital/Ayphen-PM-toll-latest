...
router.put('/:id/status/:statusId', async (req, res) => {
  try {
    ...
    const statusIndex = workflow.statuses.findIndex((s: any) => s.id === statusId);
    if (statusIndex === -1) {
...
router.delete('/:id/status/:statusId', async (req, res) => {
  try {
    ...
    workflow.statuses = workflow.statuses.filter((s: any) => s.id !== statusId);
    workflow.transitions = workflow.transitions.filter((t: any) => 
      t.from !== statusId && t.to !== statusId
    );
...
router.delete('/:id/transition/:transitionId', async (req, res) => {
  try {
    ...
    workflow.transitions = workflow.transitions.filter((t: any) => t.id !== transitionId);
    workflow.updatedAt = new Date().toISOString();
