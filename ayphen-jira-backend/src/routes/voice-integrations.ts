...
import { teamsBot } from '../services/teams-bot.service';
import { meetingIntegration } from '../services/meeting-integration.service';
...
router.post('/teams/messages', async (req, res) => {
  try {
    await teamsBot.processMessage(req, res);
  } catch (error: any) {
    res.status(500).json({
      success: false,
      error: error.message
...
router.post('/teams/notify', async (req, res) => {
  try {
    ...
    await teamsBot.sendIssueNotification(conversationReference, issue, action);
...
router.post('/meeting/start', async (req, res) => {
  try {
    ...
    const meeting = meetingIntegration.startMeeting(meetingId, platform, participants);
...
router.post('/meeting/segment', async (req, res) => {
  try {
    ...
    await meetingIntegration.addSegment(meetingId, speaker, text, confidence);
...
router.post('/meeting/end', async (req, res) => {
  try {
    ...
    const summary = await meetingIntegration.endMeeting(meetingId);
...
router.get('/meeting/:meetingId', async (req, res) => {
  try {
    ...
    const meeting = meetingIntegration.getActiveMeeting(meetingId);
...
router.post('/meeting/create-issues', async (req, res) => {
  try {
    ...
    const issues = await meetingIntegration.createIssuesFromActionItems(
      meetingId,
      actionItemIds
    );
...
router.get('/meeting/search', async (req, res) => {
  try {
    ...
    const results = await meetingIntegration.searchTranscripts(
      query as string,
