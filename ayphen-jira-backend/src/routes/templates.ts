...
import { descriptionTemplates } from '../services/description-templates.service';
...
router.get('/', (req, res) => {
  try {
    ...
    const filters: any = {};
    if (issueType) filters.issueType = issueType as string;
    if (category) filters.category = category as string;
    if (tags) filters.tags = (tags as string).split(',');
    if (userId) filters.userId = userId as string;
    if (teamId) filters.teamId = teamId as string;

    const templates = descriptionTemplates.getTemplates(filters);
...
router.get('/:id', (req, res) => {
  try {
    const { id } = req.params;
    const template = descriptionTemplates.getTemplate(id);
...
router.post('/:id/fill', async (req, res) => {
  try {
    ...
    const filledTemplate = await descriptionTemplates.fillTemplate(
      id,
      summary,
      context
    );
...
router.post('/', (req, res) => {
  try {
    ...
    const newTemplate = descriptionTemplates.createTemplate(template, userId);
...
router.put('/:id', (req, res) => {
  try {
    ...
    const success = descriptionTemplates.updateTemplate(id, updates);
...
router.delete('/:id', (req, res) => {
  try {
    ...
    const success = descriptionTemplates.deleteTemplate(id, userId as string);
...
router.post('/:id/rate', (req, res) => {
  try {
    ...
    const success = descriptionTemplates.rateTemplate(id, rating);
...
router.get('/stats/popular', (req, res) => {
  try {
    ...
    const templates = descriptionTemplates.getPopularTemplates(limitNum);
...
router.get('/search/query', (req, res) => {
  try {
    ...
    const templates = descriptionTemplates.searchTemplates(q as string);
