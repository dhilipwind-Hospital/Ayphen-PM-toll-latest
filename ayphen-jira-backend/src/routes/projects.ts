...
router.delete('/:id/members/:userId', async (req, res) => {
  try {
    ...
    const members = (project.memberRoles || []).filter(
      (m: any) => m.userId !== req.params.userId
    );
...
router.put('/:id/members/:userId/role', async (req, res) => {
  try {
    ...
    const members = (project.memberRoles || []).map((m: any) =>
      m.userId === req.params.userId ? { ...m, role } : m
    );
