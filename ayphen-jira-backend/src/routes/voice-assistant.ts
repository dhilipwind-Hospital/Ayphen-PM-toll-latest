...
interface VoiceCommandResponse {
  success: boolean;
  action: 'update' | 'navigate' | 'search' | 'create' | 'batch' | 'unknown';
  message: string;
  data?: any;
  updates?: any;
}
 ...
 */
async function processEnhancedCommand(
  command: string,
  context: { issueId?: string; projectId?: string; userId?: string }
 ...
 */
function handleNavigationCommand(cmd: string): VoiceCommandResponse {
  let route = '/dashboard';
 ...
 */
function isCreationCommand(cmd: string): boolean {
 ...
 */
async function handleCreationCommand(
  cmd: string,
  context: { projectId?: string; userId?: string }
): Promise<VoiceCommandResponse> {
  let type: 'bug' | 'story' | 'task' | 'epic' = 'task';
  let priority: 'highest' | 'high' | 'medium' | 'low' | 'lowest' = 'medium';

  // Detect type
  if (cmd.includes('bug')) type = 'bug';
  else if (cmd.includes('story')) type = 'story';
  else if (cmd.includes('epic')) type = 'epic';
  else if (cmd.includes('task')) type = 'task';
 ...
 */
function extractSummaryFromCommand(cmd: string, type: string): string {
 ...
 */
function isBatchCommand(cmd: string): boolean {
 ...
 */
async function processBasicCommand(command: string, issue: Issue) {
  const updates: any = {};
  let message = '';
  const cmd = command.toLowerCase().trim();
