...
interface SprintPlanningInput {
  ...
  backlogIssueIds: string[];
}

interface SprintComposition {
...
export class AISprintPlannerService {
   ...
   */
  async suggestSprintComposition(input: SprintPlanningInput): Promise<SprintComposition> {
    try {
      // Get backlog issues
...
Return ONLY a valid JSON object:
{
   ...
   */
  private fallbackSelection(issues: Issue[], capacity: number): SprintComposition {
    const sorted = issues
  ...
  async predictSprintSuccess(sprintId: string): Promise<SprintPrediction> {
    try {
      ...
      if (progressDelta > 0.2) successProbability = 0.9;
      else if (progressDelta > 0.1) successProbability = 0.75;
      else if (progressDelta > 0) successProbability = 0.6;
      else if (progressDelta > -0.1) successProbability = 0.4;
      else if (progressDelta > -0.2) successProbability = 0.25;
      else successProbability = 0.1;
      ...
      // Identify risks
      const risks: SprintPrediction['risks'] = [];
   ...
   */
  async balanceWorkload(sprintId: string): Promise<{
    assignments: Array<{
      userId: string;
      userName: string;
...
export const aiSprintPlannerService = new AISprintPlannerService();
