...
export interface VoiceAnalyticsEvent {
  ...
  userId: string;
  timestamp: number;
  eventType: 'command' | 'suggestion_used' | 'correction' | 'error' | 'session_start' | 'session_end';
  data: {
    transcript?: string;
    intent?: string;
    confidence?: number;
...
export class VoiceAnalyticsService {
  ...
  private readonly TIME_SAVED_PER_COMMAND = 0.5; // minutes

  /**
   * Track analytics event
   */
  trackEvent(event: Omit<VoiceAnalyticsEvent, 'id' | 'timestamp'>): void {
    const analyticsEvent: VoiceAnalyticsEvent = {
      id: this.generateId(),
      timestamp: Date.now(),
      ...event
    };
   ...
   * Track error
   */
  trackError(userId: string, errorType: string, details: any): void {
    this.trackEvent({
      userId,
      eventType: 'error',
      data: {
   ...
   */
  trackSuggestionUsed(userId: string, suggestion: string, source: string): void {
    this.trackEvent({
      userId,
   ...
   */
  getSummary(startDate: Date, endDate: Date, userId?: string): AnalyticsSummary {
    ...
    // Top intents
    const intentCounts: Record<string, { count: number; successful: number }> = {};
   ...
   * Get user insights
   */
  getUserInsights(userId: string, days: number = 30): UserInsights {
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - days);
    const endDate = new Date();

    const summary = this.getSummary(startDate, endDate, userId);
    const conversationStats = voiceConversationContext.getStats(userId);
   ...
   */
  exportData(startDate: Date, endDate: Date, format: 'json' | 'csv' = 'json'): string {
   ...
   */
  private eventsToCSV(events: VoiceAnalyticsEvent[]): string {
   ...
   */
  private filterEvents(startDate: Date, endDate: Date, userId?: string): VoiceAnalyticsEvent[] {
...
setInterval(() => {
  const cleaned = voiceAnalytics.clearOldEvents(90);
  if (cleaned > 0) {
