...
export class CommandAliasesService {
  ...
  private readonly SUGGESTION_THRESHOLD = 5; // Suggest alias after 5 uses
   ...
   * Get user's aliases
   */
  getUserAliases(userId: string): CommandAlias[] {
   ...
   */
  resolveAlias(userId: string, input: string): string {
  ...
  updateAlias(
    userId: string,
    aliasId: string,
    updates: Partial<Pick<CommandAlias, 'shortcut' | 'fullCommand' | 'tags'>>
  ): CommandAlias | null {
    const userAliases = this.getUserAliases(userId);
    ...
    if (updates.shortcut) alias.shortcut = updates.shortcut.toLowerCase().trim();
    if (updates.fullCommand) alias.fullCommand = updates.fullCommand.toLowerCase().trim();
    if (updates.tags) alias.tags = updates.tags;
  ...
  suggestAliases(
    userId: string,
    commandHistory: Array<{ command: string; count: number }>
  ): AliasSuggestion[] {
    ...
    for (const { command, count } of commandHistory) {
      ...
      if (count < this.SUGGESTION_THRESHOLD) continue;

      // Skip if alias already exists
      if (existingAliases.some(a => a.fullCommand === command)) continue;

      // Generate shortcut suggestion
      const shortcut = this.generateShortcut(command);

      // Check if shortcut is already taken
      if (existingAliases.some(a => a.shortcut === shortcut)) continue;
  ...
  private generateShortcut(command: string): string {
    ...
    const keyWords = words.filter(w => 
      !['to', 'the', 'a', 'an', 'and', 'or', 'in', 'on', 'at'].includes(w)
    );
   ...
   */
  getPopularAliases(limit: number = 10): Array<{
    shortcut: string;
    fullCommand: string;
    userCount: number;
    totalUsage: number;
   ...
   */
  importPopularAliases(userId: string, count: number = 5): CommandAlias[] {
    const popular = this.getPopularAliases(count);
    const existingAliases = this.getUserAliases(userId);
    const imported: CommandAlias[] = [];
   ...
   */
  getAliasStats(userId: string): {
    totalAliases: number;
    totalUsage: number;
    mostUsed: CommandAlias[];
    recentlyCreated: CommandAlias[];
    neverUsed: CommandAlias[];
  } {
    const userAliases = this.getUserAliases(userId);
   ...
   * Export aliases
   */
  exportAliases(userId: string): string {
   ...
   * Import aliases
   */
  importAliases(userId: string, aliasesJson: string): number {
    try {
      const imported = JSON.parse(aliasesJson) as CommandAlias[];
      let count = 0;
   ...
   * Search aliases
   */
  searchAliases(userId: string, query: string): CommandAlias[] {
    const userAliases = this.getUserAliases(userId);
    const queryLower = query.toLowerCase();

    return userAliases.filter(a =>
      a.shortcut.includes(queryLower) ||
   ...
   */
  clearUserAliases(userId: string): void {
    this.aliases.delete(userId);
  }
}

// Export singleton instance
export const commandAliases = new CommandAliasesService();
