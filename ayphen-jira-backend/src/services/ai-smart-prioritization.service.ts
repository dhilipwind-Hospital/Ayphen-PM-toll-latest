...
export class AISmartPrioritizationService {
   ...
   */
  async analyzePriority(issueId: string): Promise<PriorityAnalysis> {
    try {
   ...
   */
  private calculateUrgencyScore(issue: Issue): number {
   ...
   */
  private calculateBusinessValue(issue: Issue): number {
    let score = 50; // Base score
    const text = `${issue.summary} ${issue.description || ''}`.toLowerCase();
    ...
    if (issue.labels) {
      ...
      const hasValuableLabel = issue.labels.some(label => 
        valuableLabels.some(vl => label.toLowerCase().includes(vl))
      );
      if (hasValuableLabel) score += 15;
    }
   ...
   */
  private determineRiskLevel(urgencyScore: number, impactScore: number): 'critical' | 'high' | 'medium' | 'low' {
   ...
   */
  private async getAIPrioritySuggestion(
    issue: Issue,
    scores: {
      urgencyScore: number;
      impactScore: number;
  ...
  ): Promise<Omit<PriorityAnalysis, 'urgencyScore' | 'impactScore' | 'businessValue' | 'riskLevel'>> {
    try {
...
5. Technical risk`;
  ...
  private fallbackPrioritization(
    issue: Issue,
    scores: { urgencyScore: number; impactScore: number; businessValue: number; riskLevel: string }
  ): Omit<PriorityAnalysis, 'urgencyScore' | 'impactScore' | 'businessValue' | 'riskLevel'> {
   ...
   */
  private fallbackPriority(scores: { urgencyScore: number; impactScore: number; businessValue: number }): 
    'highest' | 'high' | 'medium' | 'low' | 'lowest' {
    const avgScore = (scores.urgencyScore + scores.impactScore + scores.businessValue) / 3;

    if (avgScore >= 80) return 'highest';
   ...
   */
  async bulkAnalyzePriority(issueIds: string[]): Promise<BulkPriorityResult[]> {
    const results: BulkPriorityResult[] = [];
   ...
   */
  async applyPriority(issueId: string, priority: string): Promise<Issue> {
    const issueRepo = AppDataSource.getRepository(Issue);
    const issue = await issueRepo.findOne({ where: { id: issueId } });
...
export const aiSmartPrioritizationService = new AISmartPrioritizationService();
