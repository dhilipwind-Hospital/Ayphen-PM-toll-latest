...
const ai_bug_analyzer_service_1 = require("../services/ai-bug-analyzer.service");
const router = (0, express_1.Router)();
/**
 * POST /api/bug-ai/classify/:issueId
 * Auto-classify a bug with AI analysis
 */
router.post('/classify/:issueId', async (req, res) => {
    try {
        const issueRepository = database_1.AppDataSource.getRepository(Issue_1.Issue);
        ...
        const classification = await ai_bug_analyzer_service_1.aiBugAnalyzer.classifyBug(issue);
 ...
 */
router.post('/find-similar/:issueId', async (req, res) => {
    try {
        const issueRepository = database_1.AppDataSource.getRepository(Issue_1.Issue);
        const issue = await issueRepository.findOne({
            where: { id: req.params.issueId }
        ...
        const similarBugs = await ai_bug_analyzer_service_1.aiBugAnalyzer.findSimilarBugs(issue, allIssues);
        res.json({
            success: true,
            issueKey: issue.key,
            similarBugs,
 ...
 */
router.post('/root-cause/:issueId', async (req, res) => {
    try {
        const issueRepository = database_1.AppDataSource.getRepository(Issue_1.Issue);
        const issue = await issueRepository.findOne({
            where: { id: req.params.issueId }
        ...
        const analysis = await ai_bug_analyzer_service_1.aiBugAnalyzer.analyzeRootCause(issue, stackTrace);
        res.json({
            success: true,
            issueKey: issue.key,
            analysis
 ...
 */
router.post('/generate-tests/:issueId', async (req, res) => {
    try {
        const issueRepository = database_1.AppDataSource.getRepository(Issue_1.Issue);
        const issue = await issueRepository.findOne({
            where: { id: req.params.issueId }
        ...
        const testCases = await ai_bug_analyzer_service_1.aiBugAnalyzer.generateTestCases(issue);
        res.json({
            success: true,
            issueKey: issue.key,
            testCases,
 ...
 */
router.post('/predict-impact/:issueId', async (req, res) => {
    try {
        const issueRepository = database_1.AppDataSource.getRepository(Issue_1.Issue);
        const issue = await issueRepository.findOne({
            where: { id: req.params.issueId }
        ...
        const impact = await ai_bug_analyzer_service_1.aiBugAnalyzer.predictImpact(issue);
        res.json({
            success: true,
            issueKey: issue.key,
            impact
 ...
 */
router.post('/full-analysis/:issueId', async (req, res) => {
    try {
        const issueRepository = database_1.AppDataSource.getRepository(Issue_1.Issue);
        const issue = await issueRepository.findOne({
            where: { id: req.params.issueId }
        ...
        const [classification, similarBugs, rootCause, testCases, impact] = await Promise.all([
            ai_bug_analyzer_service_1.aiBugAnalyzer.classifyBug(issue),
            ai_bug_analyzer_service_1.aiBugAnalyzer.findSimilarBugs(issue, allIssues),
            ai_bug_analyzer_service_1.aiBugAnalyzer.analyzeRootCause(issue),
            ai_bug_analyzer_service_1.aiBugAnalyzer.generateTestCases(issue),
            ai_bug_analyzer_service_1.aiBugAnalyzer.predictImpact(issue)
        ]);
        res.json({
            success: true,
            issueKey: issue.key,
