...
const jira_sync_service_1 = require("../services/jira-sync.service");
const context_collector_service_1 = require("../services/context-collector.service");
...
const jiraSyncService = new jira_sync_service_1.JiraSyncService();
// Helper function to calculate story points
function calculateStoryPoints(story) {
...
async function autoCreateSuites(requirementId, storyId, testCases) {
    const story = await storyRepo.findOne({ where: { id: storyId } });
    if (!story)
        return;
    const categories = ['smoke', 'sanity', 'regression'];
    for (const category of categories) {
        const casesInCategory = testCases.filter(tc => tc.categories && tc.categories.includes(category));
...
router.post('/complete', async (req, res) => {
    try {
        ...
        if (requirement.epicKey && requirement.projectId) {
            ...
            fullContext = await context_collector_service_1.contextCollector.collectFullContext(requirement.epicKey, requirement.projectId);
            if (fullContext) {
        ...
        let generated;
        try {
            generated = await retryWithBackoff(() => fullContext
        ...
        for (const storyData of generated.uiStories || []) {
            ...
            const testCasesData = await retryWithBackoff(() => openaiService.generateTestCases(story));
        ...
        if (requirement.projectId && requirement.epicKey) {
            try {
                ...
                // Sync Epic to Jira
                const epicIssue = await jiraSyncService.syncEpicToJira(requirement, requirement.projectId, userId);
                // Sync all Stories to Jira
                const syncResult = await jiraSyncService.syncAllStoriesToJira(allStories, requirement.epicKey, requirement.projectId, userId);
                jiraSyncResult = {
