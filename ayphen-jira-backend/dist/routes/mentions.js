    ...
    try {
        ...
        for (const mention of mentions) {
            ...
            if (user && user.id !== mentionedBy) {
                ...
                index_1.websocketService?.emitNotification(user.id, {
                    id: notification.id,
                ...
                if (user.emailNotifications) {
                    await email_service_1.emailService.sendEmail(user.email, `You were mentioned in ${issue?.key}`, `${mentionedByUser?.name} mentioned you:\n\n"${content}"\n\nView: ${process.env.CORS_ORIGIN}/issue/${issue?.key}`);
                }
                processedCount++;
            }
        }
