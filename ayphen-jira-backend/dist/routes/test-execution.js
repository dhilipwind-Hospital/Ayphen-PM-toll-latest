...
const TestCycle_1 = require("../entities/TestCycle");
const TestData_1 = require("../entities/TestData");
const TestDefectLink_1 = require("../entities/TestDefectLink");
...
router.get('/runs', async (req, res) => {
    try {
        ...
        if (suiteId)
            query.andWhere('run.suiteId = :suiteId', { suiteId });
        if (cycleId)
            query.andWhere('run.cycleId = :cycleId', { cycleId });
        if (status)
            query.andWhere('run.status = :status', { status });
        if (environment)
            query.andWhere('run.environment = :environment', { environment });
...
router.get('/results', async (req, res) => {
    try {
        ...
        if (testRunId)
            query.andWhere('result.testRunId = :testRunId', { testRunId });
        if (testCaseId)
            query.andWhere('result.testCaseId = :testCaseId', { testCaseId });
        if (status)
            query.andWhere('result.status = :status', { status });
...
router.post('/results/:id/create-defect', async (req, res) => {
    try {
        ...
        const testDefectLinkRepo = database_1.AppDataSource.getRepository(TestDefectLink_1.TestDefectLink);
        ...
        if (testCase.storyId) {
            const aiStoryRepo = database_1.AppDataSource.getRepository(AIStory_1.AIStory);
...
router.post('/cycles', async (req, res) => {
    try {
        const testCycleRepo = database_1.AppDataSource.getRepository(TestCycle_1.TestCycle);
...
router.get('/cycles', async (req, res) => {
    try {
        ...
        if (projectId)
            query.andWhere('cycle.projectId = :projectId', { projectId });
        if (status)
...
router.post('/test-data', async (req, res) => {
    try {
        const testDataRepo = database_1.AppDataSource.getRepository(TestData_1.TestData);
...
router.get('/test-data', async (req, res) => {
    try {
        ...
        if (testCaseId)
            query.andWhere('data.testCaseId = :testCaseId', { testCaseId });
        if (environment)
            query.andWhere('data.environment = :environment', { environment });
        if (projectId)
            query.andWhere('data.projectId = :projectId', { projectId });
