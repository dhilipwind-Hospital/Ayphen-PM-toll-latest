...
const voice_batch_operations_service_1 = require("../services/voice-batch-operations.service");
...
router.post('/batch/preview', async (req, res) => {
    try {
        ...
        const preview = await voice_batch_operations_service_1.voiceBatchOperations.previewBatch(operation);
        res.json({
            success: true,
            preview
        });
...
router.post('/batch/execute', async (req, res) => {
    try {
        ...
        const result = await voice_batch_operations_service_1.voiceBatchOperations.executeBatch(operation);
        // Track analytics
        if (req.body.userId) {
            voice_analytics_service_1.voiceAnalytics.trackCommand(req.body.userId, `Batch: ${operation.type}`, 'batch_operation', 1.0, result.success, result.executionTime, 'batch');
        }
        res.json({
            success: true,
            result
...
router.post('/batch/parse', async (req, res) => {
    try {
        ...
        const operation = await voice_batch_operations_service_1.voiceBatchOperations.parseVoiceCommand(command, context);
        if (!operation) {
            return res.json({
                success: false,
...
router.post('/analytics/track', async (req, res) => {
    try {
        ...
        voice_analytics_service_1.voiceAnalytics.trackEvent({
            userId,
            eventType,
            data: data || {}
        });
...
router.get('/analytics/summary', async (req, res) => {
    try {
        ...
        const summary = voice_analytics_service_1.voiceAnalytics.getSummary(start, end, userId);
        res.json({
            success: true,
            summary
        });
...
router.get('/analytics/insights/:userId', async (req, res) => {
    try {
        ...
        const insights = voice_analytics_service_1.voiceAnalytics.getUserInsights(userId, days ? parseInt(days) : 30);
        res.json({
            success: true,
            insights
        });
...
router.get('/analytics/realtime', async (req, res) => {
    try {
        const metrics = voice_analytics_service_1.voiceAnalytics.getRealTimeMetrics();
        res.json({
            success: true,
            metrics
        });
...
router.get('/analytics/export', async (req, res) => {
    try {
        ...
        const data = voice_analytics_service_1.voiceAnalytics.exportData(start, end, exportFormat);
        if (exportFormat === 'csv') {
