...
const ai_test_insights_service_1 = require("../services/ai-test-insights.service");
...
router.post('/analyze-flaky-test', async (req, res) => {
    try {
        ...
        const analysis = await ai_test_insights_service_1.aiTestInsightsService.analyzeFlakyTest(testCase, executionHistory);
        res.json(analysis);
    }
    catch (error) {
...
router.post('/predict-failure', async (req, res) => {
    try {
        ...
        const prediction = await ai_test_insights_service_1.aiTestInsightsService.predictTestFailure(testCase, recentChanges);
        res.json(prediction);
    }
    catch (error) {
...
router.post('/suggest-optimization', async (req, res) => {
    try {
        ...
        const suggestions = await ai_test_insights_service_1.aiTestInsightsService.suggestTestOptimization(testCase, executionTime);
...
router.post('/generate-test-data', async (req, res) => {
    try {
        ...
        const testData = await ai_test_insights_service_1.aiTestInsightsService.generateTestData(testCase, environment);
        res.json(testData);
    }
    catch (error) {
...
router.post('/identify-coverage-gaps', async (req, res) => {
    try {
        ...
        const analysis = await ai_test_insights_service_1.aiTestInsightsService.identifyMissingCoverage(requirements, tests);
        res.json(analysis);
    }
    catch (error) {
...
router.post('/chat', async (req, res) => {
    try {
        ...
        const answer = await ai_test_insights_service_1.aiTestInsightsService.answerQuestion(question, context);
