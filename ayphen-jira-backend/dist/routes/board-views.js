...
const BoardView_1 = require("../entities/BoardView");
const router = (0, express_1.Router)();
const boardViewRepo = database_1.AppDataSource.getRepository(BoardView_1.BoardView);
...
router.post('/', async (req, res) => {
    try {
        ...
        if (isDefault) {
            await boardViewRepo.update({ userId, isDefault: true }, { isDefault: false });
        }
...
router.patch('/:id', async (req, res) => {
    try {
        ...
        if (isDefault && !view.isDefault) {
            await boardViewRepo.update({ userId: view.userId, isDefault: true }, { isDefault: false });
        }
        const updates = {};
        if (name !== undefined)
            updates.name = name;
