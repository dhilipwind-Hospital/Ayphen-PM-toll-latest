...
router.post('/', async (req, res) => {
    ...
    `);
       ...
       RETURNING *`, [name, description, projectId, userId]);
        res.json(result[0]);
    }
    catch (error) {
...
router.get('/:id', async (req, res) => {
    try {
        const suite = await database_1.AppDataSource.query(`SELECT * FROM test_suites WHERE id = $1`, [req.params.id]);
        if (suite.length === 0) {
        ...
        let testCases = [];
        try {
            testCases = await database_1.AppDataSource.query(`SELECT mtc.* FROM manual_test_cases mtc
         JOIN suite_test_cases stc ON mtc.id = stc.test_case_id
         WHERE stc.suite_id = $1`, [req.params.id]);
        }
        catch (e) {
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const result = await database_1.AppDataSource.query(`UPDATE test_suites SET name = $1, description = $2, updated_at = NOW()
       WHERE id = $3 RETURNING *`, [name, description, req.params.id]);
        if (result.length === 0) {
            return res.status(404).json({ error: 'Test suite not found' });
        }
        res.json(result[0]);
...
router.delete('/:id', async (req, res) => {
    try {
        await database_1.AppDataSource.query(`DELETE FROM test_suites WHERE id = $1`, [req.params.id]);
...
router.post('/:id/test-cases', async (req, res) => {
    ...
    `);
       ...
       VALUES ($1, $2, NOW())`, [req.params.id, testCaseId]);
        res.json({ success: true });
    }
    catch (error) {
...
router.delete('/:id/test-cases/:testCaseId', async (req, res) => {
    try {
        await database_1.AppDataSource.query(`DELETE FROM suite_test_cases WHERE suite_id = $1 AND test_case_id = $2`, [req.params.id, req.params.testCaseId]);
        res.json({ success: true });
    }
    catch (error) {
