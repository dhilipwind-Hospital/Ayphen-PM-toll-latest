...
exports.descriptionTemplates = exports.DescriptionTemplatesService = void 0;
const axios_1 = __importDefault(require("axios"));
/**
 * Description Templates Service
 * Manages template library and AI-powered template filling
 */
class DescriptionTemplatesService {
    constructor() {
     ...
     */
    async fillTemplate(templateId, summary, context) {
        const template = this.getTemplate(templateId);
        if (!template) {
            throw new Error(`Template not found: ${templateId}`);
        }
        console.log(`🤖 Filling template: ${template.name}`);
        const filledSections = await Promise.all(template.sections.map(async (section) => {
            let content = section.defaultValue || '';
            if (section.aiGenerate) {
                content = await this.generateSectionContent(section, summary, template.category, context);
            }
            return {
                id: section.id,
                title: section.title,
     ...
     */
    async generateSectionContent(section, summary, category, context) {
     ...
     */
    buildSectionPrompt(section, summary, category, context) {
...
`;
     ...
     */
    buildFullDescription(template, sections) {
        const parts = [];
     ...
     */
    createTemplate(template, userId) {
        const newTemplate = {
            ...template,
...
exports.DescriptionTemplatesService = DescriptionTemplatesService;
exports.descriptionTemplates = new DescriptionTemplatesService();
