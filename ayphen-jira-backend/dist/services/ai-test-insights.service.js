...
exports.aiTestInsightsService = exports.AITestInsightsService = void 0;
const groq_sdk_1 = __importDefault(require("groq-sdk"));
const generative_ai_1 = require("@google/generative-ai");
// Initialize AI clients
const cerebrasClient = process.env.CEREBRAS_API_KEY ? new groq_sdk_1.default({
    apiKey: process.env.CEREBRAS_API_KEY,
    baseURL: 'https://api.cerebras.ai/v1',
}) : null;
const groqClient = process.env.GROQ_API_KEY ? new groq_sdk_1.default({
    apiKey: process.env.GROQ_API_KEY,
}) : null;
const geminiClient = process.env.GOOGLE_API_KEY || process.env.GEMINI_API_KEY
    ? new generative_ai_1.GoogleGenerativeAI(process.env.GOOGLE_API_KEY || process.env.GEMINI_API_KEY)
    : null;
/**
 * AI Test Insights Service
 * Provides AI-powered analysis and predictions for test automation
 */
class AITestInsightsService {
    /**
     * Analyze flaky tests and provide recommendations
     */
    async analyzeFlakyTest(testCase, executionHistory) {
...
Pass Rate: ${(executionHistory.filter((r) => r.status === 'passed').length / executionHistory.length * 100).toFixed(2)}%

Recent Execution Pattern:
${executionHistory.slice(0, 10).map((r, i) => `${i + 1}. ${r.status} (${r.environment}, ${r.browser})`).join('\n')}
...
Return only valid JSON, no markdown.`;
     ...
     */
    async predictTestFailure(testCase, recentChanges) {
...
Return only valid JSON, no markdown.`;
     ...
     */
    async suggestTestOptimization(testCase, executionTime) {
...
Return only valid JSON, no markdown.`;
     ...
     */
    async generateTestData(testCase, environment) {
...
Return only valid JSON, no markdown.`;
     ...
     */
    async identifyMissingCoverage(requirements, existingTests) {
...
exports.AITestInsightsService = AITestInsightsService;
exports.aiTestInsightsService = new AITestInsightsService();
