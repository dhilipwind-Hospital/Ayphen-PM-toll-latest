...
exports.aiRetrospectiveAnalyzer = exports.AIRetrospectiveAnalyzerService = void 0;
const database_1 = require("../config/database");
const Sprint_1 = require("../entities/Sprint");
const Issue_1 = require("../entities/Issue");
const axios_1 = __importDefault(require("axios"));
class AIRetrospectiveAnalyzerService {
    constructor() {
     ...
     */
    async aggregateSprintMetrics(sprintId) {
        try {
     ...
     */
    async extractSprintText(sprintId) {
        try {
            const issueRepo = database_1.AppDataSource.getRepository(Issue_1.Issue);
            const issues = await issueRepo.find({
                where: { sprintId },
...
Return ONLY valid JSON:
{
  ...
  "themes": ["collaboration", "technical challenges", "good progress"]
     ...
     */
    async generateAIReport(metrics, textData, sentiment) {
        try {
...
**COMPLETED ISSUES:**
${textData.issues.filter((i) => i.status === 'done').slice(0, 10).map((i) => `- ${i.key}: ${i.summary}`).join('\n')}

**INCOMPLETE ISSUES:**
${textData.issues.filter((i) => i.status !== 'done').slice(0, 5).map((i) => `- ${i.key}: ${i.summary}`).join('\n')}
...
Return ONLY valid JSON:
{
  ...
  "recommendations": [
     ...
     */
    async generateHistoricalTrends(currentSprintId, lookbackSprints = 5) {
        try {
            const sprintRepo = database_1.AppDataSource.getRepository(Sprint_1.Sprint);
            // Get current sprint
            ...
            const historicalMetrics = await Promise.all(previousSprints.map(async (sprint) => ({
            ...
            const avgBugs = historicalMetrics.length > 0
                ...
                : 0;
            // Determine trends
            const velocityTrend = currentMetrics.velocity > avgVelocity * 1.1 ? 'improving' :
                currentMetrics.velocity < avgVelocity * 0.9 ? 'declining' : 'stable';
            const completionRateTrend = currentMetrics.completionRate > avgCompletionRate * 1.1 ? 'improving' :
                currentMetrics.completionRate < avgCompletionRate * 0.9 ? 'declining' : 'stable';
            const bugTrend = currentMetrics.bugsRaised < avgBugs * 0.9 ? 'improving' :
                currentMetrics.bugsRaised > avgBugs * 1.1 ? 'worsening' : 'stable';
            // Generate AI insights
            const insights = await this.generateTrendInsights(currentMetrics, historicalMetrics, { velocityTrend, bugTrend, completionRateTrend }, avgVelocity, avgCompletionRate, avgBugs);
            return {
     ...
     */
    async generateTrendInsights(current, historical, trends, avgVelocity, avgCompletionRate, avgBugs) {
...
exports.AIRetrospectiveAnalyzerService = AIRetrospectiveAnalyzerService;
// Export singleton instance
exports.aiRetrospectiveAnalyzer = new AIRetrospectiveAnalyzerService();
